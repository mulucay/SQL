...
--veli ismi Hasan olan olan datayi silelim
DELETE FROM ogrenciler9 WHERE veli_isim='Hasan';

--TRUNCATE--
--Bir tablodaki tüm verileri geri alamayacağımız sekilde siler. sartlı silme yapmaz
TRUNCATE TABLE ogrenciler9

--ON DELETE CASCADE--Parent tablodan veri silebiliriz
DROP TABLE if exists talebeler--eger tablo varsa tabloyu siler

CREATE TABLE talebeler ( id CHAR(3) primary key, 
isim VARCHAR(50), 
veli_isim VARCHAR(50), 
yazili_notu int );
...
--notlar1 tablosundan talebe_id'si 123 olan datayi silelim
...
--NOT IN--Yazdiğimiz verilerin dışındakileri getirir
SELECT * FROM musteriler WHERE urun_isim NOT IN('Orange','Apple','Apricot');

--BETWEEN CONDITION
--musteriler tablosundan urun_id'si 20 ila 40arasında olan tum verileri  listeleyiniz
Select*from musteriler where urun_id>=20 and urun_id<=40;

--BETWEEN ile yapalim
Select*from musteriler where urun_id Between 20 and 40;

--NOT BETWEEN--aralıklar dışındakileri alıyor 20 ve 40 dahil degil
...
--marka_id’si 101’den büyük olan marka çalişanlarinin isim, 
--maaş ve şehirlerini listeleyiniz.
...
--Subquery içinde kullanilabilir.
...
-- Calisanlar toplasundan minumum maası listeleyelim
SELECT min(maas) AS minumum_maas FROM calisanlar2;

--calisanlar2 tablosundaki maaslarin toplamini listeleyiniz
SELECT sum(maas) FROM calisanlar2;

--calisanlar2 tablosundaki maaslarin ortalamasını listeleyiniz
SELECT avg(maas) FROM calisanlar2;
SELECT round(avg(maas)) FROM calisanlar2;--round virgulden sonrasini atiyor
SELECT round(avg(maas),2) FROM calisanlar2;--virgulden sonra 2 basamak aliyor

--calisanlar2 tablosundaki maas alanlarin sayisini listeleyiniz
...
--AGGREGATE METOTLARDA SUBQUERIES
...
(SELECT sum(maas) as maas_toplami from calisanlar2 where marka_isim=isyeri)
FROM markalar;
...
--VIEW KULLANIMI--yukaridaki sorguyu bir isme atama
